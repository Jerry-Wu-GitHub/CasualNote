...
def secretly_start(path):
    '''在后台运行.py文件'''
    import subprocess
    subprocess.Popen(['python', path], stdout = subprocess.PIPE, stderr = subprocess.PIPE, creationflags = subprocess.CREATE_NO_WINDOW)
...
def save(note_path, the_day:datetime.date):
	...
	saved_path = dirpath + '\\' + str(the_day)+'.md'
	saved_file = open(saved_path, mode = 'w', encoding = 'utf-8')
	...
	saved_file = open(saved_path, mode = 'r', encoding = 'utf-8')
	content = saved_file.readlines()
	saved_file.close()
	if not ''.join(content).strip():
		os.remove(saved_path)

	if not os.path.isdir('note.assets'): # note.assets 文件夹不存在
		os.mkdir('note.assets')
	elif os.listdir('note.assets'):
...
name = '历史随记'
filename = [f for f in os.listdir() if (f[:len(name)] == name and f[-3:] == '.py')]
if filename:
    path = os.path.abspath(max(filename))
