...
for year in os.listdir('previous')[::-1]:
	year_path = os.path.join('previous', year)
	file.write('\n\n## [%s 年](%s)\n\n'%(year, year_path))
	for month in os.listdir(year_path)[::-1]:
		month_path = os.path.join('previous', year, month)
		file.write('---\n\n### [%s 月](%s)\n*共%d项*\n\n'%(month, month_path, len(os.listdir(month_path))))
		for date in [f for f in os.listdir(month_path) if os.path.isfile(os.path.join(month_path, f)) and f[-3:] == '.md'][::-1]:
